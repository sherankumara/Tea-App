...
import { 
  LayoutDashboard, PlusCircle, FileText, Sprout, TrendingUp, TrendingDown, Wallet, Trash2, Coins, AlertCircle, Lock, Settings, Building2, Factory, CalendarDays, Bell, Check, X, BellRing, UserCheck, ShieldCheck, LogOut, Sparkles, Loader2, MapPin, Pencil, Save, Camera, KeyRound, Download, BrainCircuit
} from 'lucide-react';
...
const askGemini = async (prompt) => {
  try {
    ...
    return "ක්ෂමාවන්න, තාක්ෂණික දෝෂයක් නිසා සම්බන්ධ විය නොහැක. කරුණාකර මද වේලාවකින් උත්සාහ කරන්න.";
  }
};
...
const DashboardView = ({records, plots, reminders, onUpdateReminder}) => {
  ...
  const handleAI = async () => {
    ...
    const prompt = `Analyze tea estate data for ${getMonthName(m)}. Harvest: ${stats.h}kg, Income: ${formatLKR(stats.i)}, Expenses: ${formatLKR(stats.e)}, Profit: ${formatLKR(stats.i-stats.e)}. Provide 3 bullet points in Sinhala language on how to improve profit.`;
    const text = await askGemini(prompt);
    sAiRes(text); sAiLoad(false);
  };

  return <div className="space-y-6">{due.length>0 && <div className="bg-blue-50 p-4 rounded-lg border-l-4 border-blue-500"><div className="flex items-center gap-3 mb-2"><BellRing className="text-blue-600"/><h3 className="font-bold text-blue-800">මතක් කිරීම්!</h3></div>{due.map(d=><div key={d.id} className="flex justify-between items-center bg-white p-2 rounded mt-1 text-sm shadow-sm"><p><strong>{formatDate(d.date)}</strong> පොහොර යෙදීම.</p><button onClick={()=>onUpdateReminder(d.id,'completed')} className="text-blue-600 font-bold bg-blue-50 px-3 py-1 rounded hover:bg-blue-100">හරි</button></div>)}</div>}<div className="flex justify-between items-center"><h2 className="font-bold text-lg">{getMonthName(m)}</h2><div className="flex gap-2"><input type="month" value={m} onChange={e=>sM(e.target.value)} className="border p-1 rounded"/><select value={p} onChange={e=>sP(e.target.value)} className="border p-1 rounded"><option value="all">සියල්ල</option>{plots.map(pl=><option key={pl.id} value={pl.id}>{pl.name}</option>)}</select></div></div><div className="grid grid-cols-2 md:grid-cols-4 gap-4"><StatBox t="අස්වැන්න" v={stats.h.toFixed(1)+" kg"} c="bg-green-500"/><StatBox t="ආදායම" v={formatLKR(stats.i)} c="bg-blue-500"/><StatBox t="වියදම" v={formatLKR(stats.e)} c="bg-red-500"/><StatBox t="ලාභය" v={formatLKR(stats.i-stats.e)} c="bg-emerald-600"/></div>{stats.p>0 && <div className="bg-yellow-50 p-2 rounded text-yellow-700 text-sm">මිල නොදැමූ දළු: {stats.p} kg</div>}<div className="bg-gradient-to-r from-purple-50 to-indigo-50 p-4 rounded-xl border border-purple-100"><div className="flex justify-between items-center mb-3"><h3 className="font-bold text-purple-800 flex items-center gap-2"><Sparkles size={18}/> Gemini විශ්ලේෂණය</h3><button onClick={handleAI} disabled={aiLoad} className="bg-purple-600 text-white px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 disabled:opacity-50">{aiLoad?<Loader2 className="animate-spin" size={14}/>:<BrainCircuit size={14}/>} විමසන්න</button></div>{aiRes && <div className="text-sm text-purple-900 bg-white p-3 rounded-lg shadow-sm whitespace-pre-line leading-relaxed">{aiRes}</div>}</div><div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200"><h3 className="font-bold text-gray-600 mb-4">පසුගිය මාස 12</h3><div className="h-64"><ResponsiveContainer width="100%" height="100%"><LineChart data={yearlyData}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="name" fontSize={10}/><YAxis fontSize={10}/><Tooltip/><Legend/><Line type="monotone" dataKey="Harvest" stroke="#10B981" strokeWidth={2} dot={false}/><Line type="monotone" dataKey="Profit" stroke="#3B82F6" strokeWidth={2} dot={false}/></LineChart></ResponsiveContainer></div></div></div>;
};
...
const HistoryView = ({records, onDelete, onUpdate, plots, factories}) => {
  ...
  const downloadCSV = () => { if(!recs.length) return alert("දත්ත නොමැත"); const h = ["Date","Plot","Factory","Harvest","Expenses","Income","Profit","Notes"]; const r = recs.map(r => [r.date,r.plotName,r.factoryName,r.harvest,r.expenses,r.income,r.profit,r.notes||'']); const c = [h.join(","), ...r.map(e => e.join(","))].join("\n"); const l = document.createElement("a"); l.href = URL.createObjectURL(new Blob([c], {type:"text/csv;charset=utf-8;"})); l.download = `Tea_${m}.csv`; document.body.appendChild(l); l.click(); };
  return <div className="space-y-4"><div className="flex justify-between items-center bg-white p-3 rounded shadow"><div className="flex items-center gap-2"><label>මාසය:</label><input type="month" value={m} onChange={e=>sM(e.target.value)} className="border p-1 rounded"/></div><button onClick={downloadCSV} className="bg-blue-50 text-blue-600 px-3 py-1 rounded flex items-center gap-2 text-sm font-bold"><Download size={16}/> CSV</button></div>{recs.length===0 ? <div className="text-center py-10 text-gray-400">දත්ත නැත</div> : <div className="bg-white rounded shadow overflow-x-auto"><table className="w-full text-sm text-left"><thead className="bg-gray-100 text-xs uppercase"><tr><th className="p-3">දිනය</th><th className="p-3">අස්වැන්න</th><th className="p-3">ආදායම</th><th className="p-3 text-center">ක්‍රියා</th></tr></thead><tbody>{recs.map(r=><tr key={r.id} className="border-t"><td className="p-3 font-bold">{formatDate(r.date)}<div className="text-xs font-normal text-gray-500">{r.plotName}</div></td><td className="p-3 text-green-700 font-bold">{r.harvest} kg</td><td className="p-3">{r.hasPrice?formatLKR(r.income):<span className="text-xs bg-yellow-200 px-1 rounded">Pending</span>}</td><td className="p-3 flex justify-center gap-3"><button onClick={()=>sEditRec(r)} className="text-blue-500"><Pencil size={16}/></button><button onClick={()=>onDelete(r.id)} className="text-red-500"><Trash2 size={16}/></button></td></tr>)}</tbody></table></div>}{editRec && <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"><div className="bg-white p-6 rounded-xl w-full max-w-md max-h-[90vh] overflow-y-auto"><h3 className="font-bold text-lg mb-4">සංස්කරණය</h3><form onSubmit={(e)=>{e.preventDefault();onUpdate(editRec);sEditRec(null)}} className="space-y-3"><div><label className="text-xs">දිනය</label><input type="date" value={editRec.date} onChange={e=>sEditRec({...editRec,date:e.target.value})} className="w-full border p-2 rounded"/></div><div><label className="text-xs">ඉඩම</label><select value={editRec.plotId} onChange={e=>sEditRec({...editRec,plotId:e.target.value})} className="w-full border p-2 rounded">{plots.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></div><div><label className="text-xs">දළු (KG)</label><input type="number" step="0.1" value={editRec.harvestAmount} onChange={e=>sEditRec({...editRec,harvestAmount:Number(e.target.value)})} className="w-full border p-2 rounded bg-green-50 font-bold"/></div><div><label className="text-xs">සටහන්</label><textarea value={editRec.notes} onChange={e=>sEditRec({...editRec,notes:e.target.value})} className="w-full p-2 border rounded"/></div><div className="flex gap-2 pt-2"><button type="button" onClick={()=>sEditRec(null)} className="flex-1 bg-gray-200 py-2 rounded font-bold">අවලංගු</button><button type="submit" className="flex-1 bg-blue-600 text-white py-2 rounded font-bold">සුරකින්න</button></div></form></div></div>}</div>;
};
